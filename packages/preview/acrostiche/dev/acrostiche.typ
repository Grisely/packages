...
#let highlight-words(acr, def) = {
  ...
  for i in range(def.len()) {
    ...
    // highlight each letter in the long form present in the acronym by performing
  ...
  // If there are still letters to match in the acronym,
  // perform a case-insensitive search
  // This is necessary to handle acronyms like Vit - Vision Transformer
  if acr_idx < acr.len() {
    for i in range(def.len()) {
      let letter = def.at(i)
...
#let display-def(acr, plural: false, highlight: false) = {
  ...
  context {
    ...
    if acr in acronyms {
      ...
      if type(defs) == str {
        ...
        if highlight {
          highlight-words(acr, defs)
        } else {
          // All is good, we return the definition found as the singular version
          defs
        }
...
#let acr(acr, plural: false, highlight: false) = {
  ...
  context {
    ...
    if acr in data {
      ...
      if data.at(acr).at(1) {
        ...
        [#display-def(acr, plural: plural, highlight: highlight)~(#short)]
      }
    } else {
...
#let acrpl(acronym, highlight: false) = { acr(acronym, plural: true, highlight: highlight) } // argument renamed acronym to differentiate with function acr

#let acrfull(acr, highlight: false) = {
  //Intentionally display an acronym in its full form. Do not update state.
  [#display-def(acr, plural: false, highlight: highlight) (#display-short(acr))]
}

#let acrfullpl(acr, highlight: false) = {
  //Intentionally display an acronym in its full form in plural. Do not update state.
  [#display-def(acr, plural: true, highlight: highlight) (#display-short(acr, plural: true))]
}
...
#let print-index(
  ...
  // alternative-style: if true, displays an alternative list of acronyms that groups acronyms by their initial
  // highlight: if true, highlights the letters of the acronym in the long form
  ...
  context {
    ...
    grid(
      ...
      ..for acr in acr-list {
        if alternative-style and (last_letter == none or (last_letter != upper(acr.at(0)))) {
          (
            [#strong(acr.at(0))],
            [],
          )
        ...
        if clickable and (not index.get()) {
          (
            ...
            display-def(acr, plural: false, highlight: highlight),
          )
        } else {
          (
            [*#display-short(acr, plural: false)#delimiter*],
            display-def(acr, plural: false, highlight: highlight),
          )
        }
      }
